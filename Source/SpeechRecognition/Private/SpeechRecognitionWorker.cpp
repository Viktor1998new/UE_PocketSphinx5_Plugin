...
void FSpeechRecognitionWorker::AddWords(TArray<FRecognitionPhrase> dictionaryList) {
	keywords.clear();
	for (auto It = dictionaryList.CreateConstIterator(); It; ++It)
	{
		...
		ret = keywords.insert(pair<string, char*>(wordStr, tolerance));
		if (ret.second == false) {
			keywords.erase(wordStr);
			keywords.insert(pair<string, char*>(wordStr, tolerance));
		}
	}
}
...
void FSpeechRecognitionWorker::SetLanguage(ESpeechRecognitionLanguage language) {
	...
	UE_LOG(LogTemp, Warning, TEXT("Path: %s"), *contentPath);
	...
	case ESpeechRecognitionLanguage::VE_Russia:
		langStr = (char*)"ru";
		break;
...
void FSpeechRecognitionWorker::InitConfig() {
	...
	FString LogText = FString(modelPath.c_str());
	UE_LOG(SpeechRecognitionPlugin, Log, TEXT("%s"),*LogText);
	...
	while (StopTaskCounter.GetValue() == 0) {
		...
		if (initRequired) {
			...
			if (detectionMode == ESpeechRecognitionMode::VE_KEYWORD) {
				...
				ps_set_keyphrase(ps, "keyphrase_search", const_copy);
